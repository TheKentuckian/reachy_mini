#!/bin/bash
# Disable the central signaling relay (the fork default). Run this ON THE ROBOT.
#
# Writes REACHY_CENTRAL_RELAY=0 to /etc/reachy-mini/relay.env (read by
...
printf 'REACHY_CENTRAL_RELAY=0\n' | sudo tee "$RELAY_ENV_FILE" > /dev/null
echo "Wrote REACHY_CENTRAL_RELAY=0 to $RELAY_ENV_FILE"

echo "Restarting reachy-mini-daemon..."
sudo systemctl restart reachy-mini-daemon

cat <<'EOF'
Central signaling relay is now OFF (the fork default).
