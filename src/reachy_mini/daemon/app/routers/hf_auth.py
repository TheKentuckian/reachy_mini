...
# module — imported lazily (fork: the relay module pulls in aiohttp +
# websockets, which we keep off the daemon startup path).


def _central_robot_status_url() -> str:
    from reachy_mini.media.central_signaling_relay import CENTRAL_SIGNALING_SERVER

    return f"{CENTRAL_SIGNALING_SERVER}/api/robot-status"
...
async def begin_oauth(request: Request) -> RedirectResponse:
    ...
    result = _hf_auth().create_oauth_session(
