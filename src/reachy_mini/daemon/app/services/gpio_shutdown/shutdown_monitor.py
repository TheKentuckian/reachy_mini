...
def _pid_comm(pid: int) -> str:
    """Return the process name of ``pid`` from /proc, or "" if unreadable."""
    try:
...
def _stop_daemon_and_wait() -> None:
    ...
    # SIGUSR1 is only meaningful if MainPID is the Python daemon. If the
    # unit still runs a launcher.sh without `exec` (stale unit / site-packages
    # copy), MainPID is bash, whose default SIGUSR1 disposition is to die —
    # we'd then poweroff while python is still parking the head. Fall back
    # to a synchronous `systemctl stop` (SIGTERM -> goto_sleep_on_stop=True).
    comm = _pid_comm(pid)
    if not comm.startswith("python"):
        _log(
            f"{DAEMON_UNIT} MainPID {pid} is '{comm}', not python "
            "(launcher without exec?); using systemctl stop instead of SIGUSR1"
        )
        try:
            subprocess.run(
                ["sudo", "systemctl", "stop", DAEMON_UNIT],
                timeout=DAEMON_STOP_TIMEOUT_S,
            )
            _log(f"{DAEMON_UNIT} stopped")
        except (subprocess.SubprocessError, FileNotFoundError) as exc:
            _log(f"systemctl stop {DAEMON_UNIT} failed: {exc}; halting anyway")
        return
