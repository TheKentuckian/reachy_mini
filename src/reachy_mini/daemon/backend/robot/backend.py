...
class RobotBackend(Backend):
    ...
    def set_motor_torque_ids(self, ids: list[str], on: bool) -> None:
        ...
        # A re-torqued servo latches Goal := Present, so an unchanged target
        # must be re-sent on the next tick: drop the redundant-write cache.
        self._last_sent_head_positions = None
        self._last_sent_antenna_positions = None
    ...
    def write_raw_packet(self, packet: bytes) -> bytes:
        ...
        # A raw packet may have changed goal/torque state behind our back.
        self._last_sent_head_positions = None
        self._last_sent_antenna_positions = None
        return result
