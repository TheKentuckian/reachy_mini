...
| `launcher.sh` (relay.env sourcing, rfkill) | Yes | Units point at the checkout via drop-ins (§1) |
| `gpio-shutdown-daemon.service` + monitor | Yes — `src/.../services/gpio_shutdown/` | Reinstall via its `install_service.sh`; repoint via §1 |
| `/etc/reachy-mini/relay.env` | **No** | `REACHY_CENTRAL_RELAY=0`; also a general `REACHY_*` env file for the daemon |
| systemd drop-ins `*.service.d/checkout-paths.conf` | **No** | Written by `scripts/repoint_systemd_units.sh` (§1) |
...
| Central relay disabled by default | Yes — see `COMPATIBILITY.md` | `scripts/relay_on.sh` / `relay_off.sh` |

---

## Post-reflash checklist

1. Install the fork from source into **both** venvs (no `uv` on the robot; clear
   the cache first — the SD card is small):
   ```bash
   rm -rf ~/.cache/pip ~/.cache/uv
   cd ~/reachy_mini
   /venvs/mini_daemon/bin/pip install -e .
   /venvs/apps_venv/bin/pip install -e .
   ```

2. Install the daemon + GPIO units, then repoint them at the checkout (§1):
   ```bash
   src/reachy_mini/daemon/app/services/wireless/install_service.sh
   src/reachy_mini/daemon/app/services/gpio_shutdown/install_service.sh
   scripts/repoint_systemd_units.sh
   ```
...
## §1 — Units must exec the checkout, not site-packages

The robot runs an **editable install**, so the packaged unit files' `ExecStart`
(`/venvs/mini_daemon/lib/python3.12/site-packages/reachy_mini/...`) points at a
path that does not exist. Worse, the daemon's wireless startup checks re-copy
the packaged `reachy-mini-daemon.service` / `gpio-shutdown-daemon.service` into
`/etc/systemd/system` on every boot, so editing them in place does not stick.
`scripts/repoint_systemd_units.sh` writes `checkout-paths.conf` drop-ins for
`reachy-mini-daemon`, `gpio-shutdown-daemon` and `reachy-mini-bluetooth` that
override `ExecStart`/`WorkingDirectory`; drop-ins survive the re-copy.

Obsolete drop-ins from the 1.8.4-era fork (remove if present — the env vars no
longer exist): `reachy-mini-daemon.service.d/fk-skip.conf`,
`reachy-mini-daemon.service.d/no-antenna-sleep.conf`.
...
without a password.  Required for the safe-restart endpoint (issue #25) when
`pollen` does not have passwordless sudo for `systemctl`.
...
/* Allow user pollen to start/stop/reset-failed reachy-app-autostart.service without a
   password.  Required for the safe-restart endpoint (#25).  All three operations
   fall under manage-units. */
...
- **Central relay disabled** — the HuggingFace WebRTC relay is off by default;
  `scripts/relay_on.sh` / `relay_off.sh` toggle it (see `COMPATIBILITY.md`).
- **Daemon service is stock `Type=simple`** plus `TimeoutStopSec=20s` (the GPIO
  monitor waits 25 s for the daemon to park the head before powering off). A
  `Type=notify` unit left over from the 1.8.4-era fork will make systemd wait
  for a READY signal the daemon no longer sends — always reinstall the unit
  after upgrading.
- **Power button** sends SIGUSR1 to the daemon (forced safe shutdown), then
  `shutdown --poweroff`. Log: `~/gpio_shutdown.log` (10 KB cap).
